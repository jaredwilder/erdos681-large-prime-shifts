...
out = {"n": len(G), "seed": 682, "mean_lower_witnesses": float(np.mean(lo)), "mean_upper_witnesses": float(np.mean(hi)),
       "share_zero_upper": float(np.mean([h == 0 for h in hi])), "share_zero_lower": float(np.mean([l == 0 for l in lo]))}
s_ = json.dumps(out); open("wit_split.json", "w").write(s_); print(s_); print("HASH", hashlib.sha256(s_.encode()).hexdigest())
