...
for H in (401,):
    ...
    rows.append({"H": H, "odd_shifts": len(range(3, H + 1, 2)), "primes_used": len(chosen), "log_M": round(used, 3), "budget": round(budget, 3), "uncovered": len(shifts), "chosen": chosen, "prod_bound": round(0.5 * len(range(3, H + 1, 2)) * math.prod(1 - 1 / q for q in chosen), 3), "sqrtH": round(H ** .5, 2)})
    print(rows[-1])
s = json.dumps(rows); open("cover_budget_401b.json", "w").write(s)
