...
for K in range(3, 30, 2):
    ...
    for q in primes(K*K):
        ...
        dist = new
    rows.append({"K": K, "shifts": n, "delta": float(dist[full])}); print(rows[-1], flush=True)
s = json.dumps(rows); open("delta_K.json", "w").write(s); print("HASH", hashlib.sha256(s.encode()).hexdigest())
